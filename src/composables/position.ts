...
export function usePosition(panelEl: Ref<HTMLElement | undefined>, usePanelState: () => UsePanelStateReturn) {
  ...
  const bringUp = () => {
    ...
    if (state.value.minimizeInactive < 0)
      return
    if (_timer)
      clearTimeout(_timer)
    _timer = setTimeout(() => {
      isHovering.value = false
    }, +state.value.minimizeInactive || 0)
  }
  ...
  const dockStyle = computed(() => {
    const style: any = {
      transform: isVertical.value
        ? `translate(${isInactive.value ? `calc(-50% ${state.value.position === 'right' ? '+' : '-'} 15px)` : '-50%'}, -50%) rotate(90deg)`
        : `translate(-50%, ${isInactive.value ? `calc(-50% ${state.value.position === 'top' ? '-' : '+'} 15px)` : '-50%'})`,
    }
    if (isDragging.value)
