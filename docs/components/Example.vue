...
<template>
  <FloatingPanelRoot v-slot="{ isDragging }" name="secondPanel" :minimize-inactive="5000">
    <FloatingPanelDock
      :class="{ '!cursor-grabbing': isDragging }"
      class="dock data-[state=inactive]:max-w-108px data-[state=inactive]:py-0.5 data-[state=inactive]:px-0 cursor-grab overflow-hidden w-227px max-w-280px py-1 pl-1 pr-5px h-65px flex items-center gap-0.5 justify-start overflow-hidden absolute left-0 top-0 translate-[-50%_-50%] rounded-20px box-border border-px dark:border-#3336 border-#efefef dark:bg-#111 bg-white backdrop-blur-10px select-none text-[color:var(--floating-dock-widget-fg)] shadow-[2px_2px_8px_var(--floating-dock-widget-shadow)]"
    >
      <FloatingPanelTrigger
        class="flex-none  w-[50px] h-[50px] flex justify-center items-center opacity-80 transition-opacity duration-[0.2s] ease-[ease-in-out] rounded-[100%] border-0 hover:opacity-100"
      >
